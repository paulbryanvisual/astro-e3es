...
- **List Card Layout Alignment**: The slanted E3 card list block styles (`ul.wp-block-list` and `ol.wp-block-list`) override all default browser and Gutenberg block padding (`padding-left: 0 !important; margin-left: 0 !important;`). To align the slanted card's left diagonal border with the other page text elements, list items (`li`) use a precise left margin of `4px` that compensates for the center-point origin `skewX(-6deg)` transformation shift.
