...
- **Interactive Region Locking**: Clicking a map region locks its selection by adding `.active` and `.locked` classes and `aria-selected="true"`. Hover state previews (`mouseenter`/`mouseleave`) are bypassed while any region is locked (`if (this.isLocked) return;`). Clicking outside the map preserves the selection (no global click listener), while clicking a different region transitions the lock state to the newly clicked region.
