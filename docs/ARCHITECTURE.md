...
- **Client Detail Spacing**: Spacing below the hero banner is handled conditionally to prevent white margin gaps when layout blocks with background colors (e.g., `.db-feature` blocks, group sections, cover overlays, or custom background colors) are placed immediately below the banner. It targets `main > :first-child:not(.db-feature):not(.has-background):not(.wp-block-group):not(.wp-block-cover)` to apply `margin-top: 4rem !important` only to standard content elements (like paragraphs).
