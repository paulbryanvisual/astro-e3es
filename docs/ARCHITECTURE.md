...
- **Client Detail Header Architecture**: Dynamically parses CPT taxonomies and metadata terms on the clients detail static paths page (`src/pages/clients/[slug].astro`), resolving a brand-styled `<header>` block for each client containing client logo, industry, region, and custom services provided tags.
...
- **Unified SVG Map Rendering**: In `wordpress.ts`, `processWordPressHtml()` replaces static map image placeholders with `TEXAS_MAP_SVG`, which contains custom inline stylesheet overrides to force all region paths (`.cls-1` through `.cls-8`) to use the brand green color, and replaces standard client dots with 5-point star polygons.
