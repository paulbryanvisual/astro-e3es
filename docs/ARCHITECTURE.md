...
## Gutenberg Editor Iframe Namespace Mangling & Scoping
- **Gutenberg editorStyles Namespace Mangling**: In modern WordPress, block editor canvas iframe styles added via the `editorStyles` setting have all selectors automatically parsed and prepended with `.editor-styles-wrapper`. If a stylesheet compiles selectors already containing `.editor-styles-wrapper`, they get mangled to `.editor-styles-wrapper .editor-styles-wrapper ...`, which fails to match elements.
- **Scoping Editor-Only Overrides**: To prevent namespace mangling while targeting editor-specific elements, styles must avoid using `.editor-styles-wrapper` at the root of nested rules. Instead, selectors are scoped using editor-only classes and attributes, such as `.is-root-container` or `[data-type="..."]`. This compiles to standard selectors that are cleanly prepended by Gutenberg's parser, resulting in valid selectors like `.editor-styles-wrapper [data-type="..."]` inside the iframe.
