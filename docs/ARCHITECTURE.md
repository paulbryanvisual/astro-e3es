...
- **1400px Max-Width Container**: Standardized outer container bounds in `SalesRepRegionSelector.astro` with `display: block; width: 100%; max-width: 1400px; margin: 40px auto; padding: 0 40px; box-sizing: border-box;` for responsive centering within screen constraints up to 1400px canvas width. Enforced matching `max-width: 1400px !important; margin-left: auto !important; margin-right: auto !important;` in `desktop.scss` and `mobile.scss` targeting `e3-sales-rep-selector`, `.sales-rep-selector`, `.wp-block-e3es-sales-rep-region`, and wrapper elements.
- **SVG Layer Elevation Architecture**: Visual stacking order in SVG maps is dictated by DOM node order. `SalesRepRegionSelector` uses `elevateRegion(regionElement)` to move target regions to the end of `#Layer_1` via `appendChild()`. To prevent DOM thrashing (previously 17 node moves per hover cycle), `elevateRegion` checks `parent.lastElementChild !== regionElement` before performing a single O(1) DOM append, and `restoreRegionOrder()` checks `needsSort` before running array sorting. Native event listeners (`addEventListener`) and class-based CSS selectors remain attached and stable across node movements.
- **WAI-ARIA & Keyboard Focus Architecture**: SVG region elements use `role="button"` with `aria-pressed="true"|"false"` toggle states and `aria-controls="find-your-rep"`. Screen reader announcements use a dedicated visually hidden live region announcer (`.sr-only.sales-rep-selector__status-announcer` with `aria-live="polite"`), preventing live-region noise during mouse hovers. DOM re-ordering is guarded with active element capturing/restoration to maintain keyboard focus, and `unlockMap()` explicitly transfers focus back to the previously locked region button (`#region-${targetRegionId}`) upon clearing selection.
- **Interactive Region Locking & Constant Scope**: Clicking a map region locks its selection by adding `.active` and `.locked` classes and `aria-pressed="true"`. Hover state previews (`mouseenter`/`mouseleave`) and JS hover methods exit immediately while locked (`if (this.isLocked) return;`), and CSS overrides prevent hover highlights on non-locked regions. Clicking outside the map or re-clicking the locked region preserves the lock (no toggle-off or outside dismiss listeners). Only explicitly clicking a different region transitions the lock to the new region. Region name mappings are unified in a top-level module constant `REGION_LABELS`.
- **Card Content & Placeholder Rules**: Representative cards omit bio text completely (displaying photo, region label, name, role, email CTA, schedule call CTA, and reset button). Standard default placeholder text is explicitly defined as `"Select your region and contact us to get started."`.
...
- **Alphabetical Service Dropdown Sorting**: Service dropdown menus generated by `buildBreadcrumbs()` in `src/lib/wordpress.ts` (both top-level root services under the "Services" breadcrumb item and child services under parent service items) are normalized using `decodeHtmlEntities()` to clean raw HTML entities (e.g. `&#038;` $\rightarrow$ `'and'`). Items are then sorted alphabetically using `localeCompare()` with `{ sensitivity: 'base', numeric: true }`. Trashed/legacy service entries (`!item.slug?.includes('trashed')`) are excluded to ensure clean, consistent navigation across all service pages.
- **Services Index Breadcrumbs**: The Services index page (`src/pages/services.astro`) renders the `Breadcrumb` component using `buildBreadcrumbs(servicesPage, allItems)`. The `buildBreadcrumbs` logic maps the top-level root services as the children dropdown of the Services page (`id === 11` or `slug === 'services'`), ensuring the dropdown is fully interactive and lists all services alphabetically.
...
- **Production Static Mode**: When built without the staging environment variable, Astro compiles the website statically (`output: 'static'`). Deployments compile all pages into optimized static HTML files.
- **Node.js Environment Compatibility & Asset Bundling**: Standard filesystem operations (`fs.readFileSync` and `path`) are unsupported in Cloudflare Workers edge runtimes. To prevent runtime failures on the staging server, the interactive Texas region map SVG is bundled directly into the compiled JavaScript bundle at build time using Vite's raw import syntax (`import rawTexasMapSvg from '../../public/Texas-Map-Editable.svg?raw'`).
- **Asynchronous Webhook Triggering**: To prevent the WordPress admin dashboard from hanging or experiencing slow page saving when publishing or updating posts/pages, the `cloudflare-deploy-trigger` WordPress plugin uses asynchronous `wp_remote_post` calls. Setting `'blocking' => false` makes webhook and GitHub API dispatch triggers fire-and-forget, returning control to the editor instantly. Manual deployments triggered from the options page pass an explicit `blocking => true` argument to preserve success/error feedback notifications.
- **Dependency Clean Install Optimization**: Wrangler and Puppeteer-Core are moved from production `dependencies` to `devDependencies` in `package.json`. This eliminates binary extraction and package download overhead in Cloudflare Pages' clean-install step, speeding up CI/CD pipeline runs.
