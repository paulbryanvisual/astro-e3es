...
## Session Wrapup: 2026-07-30T23:30:00Z - Alphabetical Service Breadcrumb Dropdown Sorting & Services Index Breadcrumbs
**Architectural Decisions:**
- Sorted services dropdown menus generated by `buildBreadcrumbs()` in `src/lib/wordpress.ts` alphabetically from A to Z using `localeCompare` with base sensitivity. Normalizes HTML entity characters (like `&#038;` and `&amp;`) to clean strings before sorting, and filters out trashed posts.
- Configured `buildBreadcrumbs()` to support a custom dropdown mapping on the Services index page (`id === 11` or `slug === 'services'`) using a `shortServiceNames` record for cleaner, compact menu labels (e.g. "HVAC & Controls" instead of full titles). Excluded chiller plants, boiler plants, and cooling towers from dropdown menus.
- Integrated the `Breadcrumb` component directly onto the Services index page (`src/pages/services.astro`) and resolved its breadcrumbs array from `servicesPage` (page ID 11) and combined pages/services lists.
