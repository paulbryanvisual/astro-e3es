...
async function runAudit() {
  ...
  for (const slug of SLUGS) {
    ...
    while ((match = pRegex.exec(liveHtml)) !== null) {
      ...
      if (pText.includes('Join Our Team') || 
          pText.includes('prague-architects') || 
          pText.includes('ALL RIGHTS RESERVED') || 
          pText.includes('Tel:') || 
          pText.includes('+7 (885)') || 
          pText.includes('Jungmannova') || 
          pText.includes('Czech Republic') || 
          pText.includes('PROJECT SCOPE') || 
          pText.includes('CONTRACT AMOUNT') || 
          pText.includes('ANNUAL SAVINGS') || 
          pText.includes('Office Locations') || 
          pText.trim() === 'K-12' || 
          pText.trim() === 'Municipal Water Quality' || 
          pText.trim() === 'Higher Education' || 
          pText.trim() === 'Healthcare') {
